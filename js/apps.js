/**
 * GetAli V2.5 - Apps Page
 * Tool apps listing with detail page navigation
 * @version 2.5.0
 */
...
class AppsStore {
  ...
  async init() {
    try {
      ...
      console.error('Failed to initialize apps store:', error);
  ...
  renderAll() {
    if (!this.sectionsContainer || !this.appsData) return;
    this.sectionsContainer.innerHTML = '';

    const featuredApp = this.appsData.categories[0].apps[0];
    this.renderFeatured(featuredApp);

    this.appsData.categories.forEach((category, catIndex) => {
      this.renderCategory(category, catIndex);
    });
  }

  renderFeatured(app) {
    const placeholder = this.generatePlaceholder(app.name, 0);
    const featuredHTML = `
      <div class="featured-card" data-app-id="${app.id}">
        <div class="featured-media">
          <img src="${app.image}" alt="${app.name}" loading="eager"
               onerror="this.src='${placeholder}'">
          <div class="featured-overlay">
            <span class="featured-badge">اختيار المحررين</span>
          </div>
        </div>
        <div class="featured-body">
          <h2 class="featured-title">${app.name}</h2>
          <div class="featured-meta">
            ...
            <span>${app.size}</span>
            <span>•</span>
            <span>${app.downloads || '1M+'} تحميل</span>
          </div>
          <p class="featured-desc">${app.description}</p>
          <div class="featured-actions">
            <button class="btn btn-primary download-app-btn" data-app-id="${app.id}">
            ...
            <a href="detail.html?id=${app.id}&type=apps" class="btn btn-secondary btn-sm">التفاصيل</a>
          </div>
        </div>
      </div>
    `;
  ...
  renderCategory(category, catIndex) {
    ...
    category.apps.forEach((app, appIndex) => {
      grid.appendChild(this.createAppCard(app, catIndex * 10 + appIndex));
    });
  }

  createAppCard(app, colorIndex) {
    const card = document.createElement('div');
    card.className = 'game-card';
    card.dataset.appId = app.id;
    card.dataset.name = app.name.toLowerCase();
    card.dataset.dev = app.developer.toLowerCase();

    const placeholder = this.generatePlaceholder(app.name, colorIndex);
    ...
    card.innerHTML = `
      <div class="game-thumb">
        ...
        <div class="game-rating">
          <span>★</span><span>${app.rating}</span>
        </div>
      </div>
      <div class="game-info">
        <div class="game-title">${app.name}</div>
        <div class="game-dev">${app.developer}</div>
        <div class="game-footer">
          <span class="game-size">${app.size}</span>
          <div class="app-card-actions">
            <a href="detail.html?id=${app.id}&type=apps" class="details-link" title="التفاصيل">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                <circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/>
              </svg>
            </a>
            <button class="download-btn" data-app-id="${app.id}" title="تحميل">
    ...
    card.addEventListener('click', (e) => {
      if (!e.target.closest('.download-btn') && !e.target.closest('.details-link')) {
        window.location.href = `detail.html?id=${app.id}&type=apps`;
      }
    });
  ...
  handleDownload(app) {
    ...
    const fullApp = this.findAppById(app.id);
    if (fullApp) this.downloadManager.startDownload(fullApp);
  }

  findAppById(id) {
    for (const category of this.appsData.categories) {
  ...
  setupEvents() {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (btn) {
        ...
        if (app) this.handleDownload(app);
      }
    });
  }
